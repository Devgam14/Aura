...
const convert = () => {
  ...
  if (rateFrom && rateTo) {
    ...
    priceDisplays[0].textContent = `1 ${fromCurrency} = ${(1 / rateFrom * rateTo).toFixed(6)} ${toCurrency}`;
    priceDisplays[1].textContent = `1 ${toCurrency} = ${(1 / rateTo * rateFrom).toFixed(6)} ${fromCurrency}`;
  }
};
